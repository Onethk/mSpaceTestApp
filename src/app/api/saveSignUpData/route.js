...
export async function POST(request) {
  try {
    ...
    console.log("existing results", existingPhoneResult);
    ...
    const subscriptionExistsQuery = "SELECT * FROM Subscription WHERE phoneNumber = ?";
    const result = await db.get(subscriptionExistsQuery, [phoneNumber]);
    console.log(result);
    if (result) {
    ...
    return NextResponse.json({ message: "Login successful", success: true });
      ...
      const insertQuery = "INSERT INTO Subscription (phoneNumber, password) VALUES (?, ?)";
      await db.run(insertQuery, [phoneNumber, password]);;
